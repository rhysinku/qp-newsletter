 ...
 *
 * Handles display for individual 'blog' custom post type items.
 *
...
// Primary category term ('blog_category-term').
$category_term = null;
$category_id = $fields['blog_category-term'] ?? get_post_meta($post_id, 'blog_category-term', true);
if (!empty($category_id)) {
  if (is_numeric($category_id)) {
    $term = get_term((int) $category_id, 'blog_category');
...
<article class="mmd-single-blog">
  <!-- Hero Banner Area -->
  <section class="mmd-single-blog__hero bg-neutral-light-grey pt-8 pb-12 border-b border-neutral-grey-200/60">
    <div class="container mx-auto px-6 max-w-[1200px]">
      ...
      <?php else : ?>
        <nav aria-label="Breadcrumb" class="mmd-breadcrumbs text-xs font-semibold text-neutral-grey-500 mb-6">
          <ol class="flex items-center flex-wrap gap-2 list-none p-0 m-0">
            ...
            <li>
              <a href="<?php echo esc_url(home_url('/blog/')); ?>" class="hover:text-primary transition-colors focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2">Blog</a>
            </li>
            <li aria-hidden="true" class="text-neutral-grey-300">/</li>
      ...
      <?php if (!empty($thumbnail_id)) : ?>
        <div class="mt-8 overflow-hidden rounded-2xl lg:rounded-3xl shadow-md border border-neutral-grey-200/50 bg-neutral-grey-100">
          <?php echo mmd_render_image_by_ris($thumbnail_id, 'blog-featured', ['w-full', 'h-auto', 'aspect-video', 'object-cover'], $title, wrap_figure: false, eager: true); ?>
        </div>
      <?php endif; ?>
  ...
  <section class="mmd-single-blog__body py-12 lg:py-16">
    <div class="container mx-auto px-6 max-w-[1200px]">
